...
from tepra import Tepra
import typing

min_width = 84
height = 64  # px
default_font = pathlib.Path('PATH_TO_A_FONT')
...
@click.option('--font', '-f', type=click.Path(exists=True, path_type=pathlib.Path), help='Path to a font file.')
@click.option(
...
def do_print(ctx, preview, font, fontsize, depth, **_):
    ...
    try:
        ...
        print('Please specify an available font.')
        sys.exit(1)
    ...
    t = Tepra()
    try:
        asyncio.run(t.run(depth, encoded))
    finally:
